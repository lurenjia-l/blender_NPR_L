...
struct Reader {
  ...
  float read_surface_depth(Header header, int2 texel, float fallback_depth) const
  {
    if (!header.use_surface_depth()) {
      return fallback_depth;
    }
    float2 data_packed = fetch_additional_data(texel).rg;
    return AdditionalInfo::unpack(data_packed).surface_depth;
  }
...
{
  [[resource_table]] const gbuffer::Reader &reader = resource_table_get(gbuffer::Reader);
  return reader.read_object_id(texel);
}
