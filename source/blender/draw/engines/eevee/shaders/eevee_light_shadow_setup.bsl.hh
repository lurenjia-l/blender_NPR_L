...
struct Resources {
                         ...
                         eShadowProjectionType projection_type)
  {
     ...
     * the inverse in this particular case. */
                       ...
                       [[global_invocation_id]] const uint3 global_id)
{
   ...
   * needs to be avoided. */
  bool valid_local = (l_idx < srt.light_cull_buf.visible_count);
  bool valid_directional = (l_idx >= srt.light_cull_buf.local_lights_len) &&
                           (l_idx < srt.light_cull_buf.items_count);
  ...
  if (is_sun_light(light.type)) {
    ...
    if (light.type == LIGHT_SUN_ORTHO) {
      srt.cascade_sync(light);
    }
    else {
      srt.clipmap_sync(light);
    }
  }
  else {
    /* Local lights. */
