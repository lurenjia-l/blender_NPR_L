...
static const EnumPropertyItem rna_enum_view_layer_native_postfx_source_items[] = {
    ...
    {VIEW_LAYER_NATIVE_POSTFX_SOURCE_DIFFUSE_LIGHT, "DIFFUSE_LIGHT", 0, "Diffuse Light", ""},
    {VIEW_LAYER_NATIVE_POSTFX_SOURCE_DIFFUSE_COLOR, "DIFFUSE_COLOR", 0, "Diffuse Color", ""},
...
enum class SceneEEVEEPerformanceString {
  ViewportSummary,
  ViewportReport,
  RenderReport,
};
...
{
  const SceneEEVEE *eevee = static_cast<const SceneEEVEE *>(ptr->data);
  return (eevee->performance_profiler_average_window > 0) ?
             eevee->performance_profiler_average_window :
             8;
}

static void rna_SceneEEVEE_performance_profiler_average_window_set(PointerRNA *ptr, int value)
{
  SceneEEVEE *eevee = static_cast<SceneEEVEE *>(ptr->data);
  eevee->performance_profiler_average_window = max_ii(value, 1);
}
...
static void rna_Scene_eevee_profiler_view_update(Main * /*bmain*/,
                                                 Scene *scene,
                                                 PointerRNA *ptr)
{
  Scene *owner_scene = id_cast<Scene *>(ptr->owner_id);
  if (owner_scene == nullptr) {
    owner_scene = scene;
  }

  WM_main_add_notifier(NC_SCENE | ND_RENDER_OPTIONS, &owner_scene->id);
...
static std::optional<std::string> rna_SceneFilterMaterial_path(const PointerRNA *ptr)
{
  ...
  BLI_str_escape(filter_material_name_esc,
                 filter_material->name,
                 sizeof(filter_material_name_esc));
...
static void rna_SceneEEVEE_filter_material_remove(ID *id,
                                                  SceneEEVEE *eevee,
                                                  ReportList *reports,
                                                  int index)
{
...
static void rna_SceneEEVEE_filter_material_move(ID *id,
                                                SceneEEVEE *eevee,
                                                ReportList *reports,
                                                int from,
                                                int to)
...
static std::optional<std::string> rna_SceneRenderTexture_path(const PointerRNA *ptr)
{
  ...
  BLI_str_escape(render_texture_name_esc, render_texture->name, sizeof(render_texture_name_esc));
...
{
  if (BLI_listbase_count(&eevee->render_textures) >=
      RNA_SCENE_EEVEE_RENDER_TEXTURE_SLOT_MAX) {
    return nullptr;
  }
...
static void rna_SceneEEVEE_render_texture_remove(ID *id,
                                                 SceneEEVEE *eevee,
                                                 ReportList *reports,
                                                 int index)
{
...
static void rna_SceneEEVEE_render_texture_move(ID *id,
                                               SceneEEVEE *eevee,
                                               ReportList *reports,
                                               int from,
                                               int to)
    ...
    PointerRNA *ptr, int *min, int *max, int * /*softmin*/, int * /*softmax*/)
{
  ...
  *min = 0;
  *max = max_ii(0, view_layer->native_postfx_outputs.count() - 1);
}
...
{
  const ViewLayerNativePostFXOutput *output = static_cast<ViewLayerNativePostFXOutput *>(ptr->data);
  const Scene *scene = reinterpret_cast<const Scene *>(ptr->owner_id);
...
#  endif
  ...
  RNA_def_property_ui_text(prop, "Outline", "Deliver screen-space outlines in a separate pass");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, "rna_ViewLayer_pass_update");
...
static void rna_def_view_layer_native_postfx_output(BlenderRNA *brna)
{
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", VIEW_LAYER_NATIVE_POSTFX_OUTPUT_ENABLED);
  ...
  RNA_def_property_boolean_sdna(prop, nullptr, "effects", VIEW_LAYER_NATIVE_POSTFX_OUTPUT_EFFECT_DOF);
...
static void rna_def_scene_eevee(BlenderRNA *brna)
{
  ...
  RNA_def_property_ui_text(prop, "Active Render Texture Index", "Index of the active render texture");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, nullptr);

  prop = RNA_def_property(srna, "use_performance_profiler", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", SCE_EEVEE_PERFORMANCE_PROFILER);
  RNA_def_property_boolean_default(prop, false);
  RNA_def_property_ui_text(
      prop,
      "Performance Profiler",
      "Collect Eevee CPU stage timings and show summaries for viewport and final render");
  RNA_def_property_update(
      prop, NC_SCENE | ND_RENDER_OPTIONS, "rna_Scene_eevee_profiler_view_update");

  prop = RNA_def_property(srna, "performance_profiler_average_window", PROP_INT, PROP_NONE);
  RNA_def_property_int_funcs(prop,
                             "rna_SceneEEVEE_performance_profiler_average_window_get",
                             "rna_SceneEEVEE_performance_profiler_average_window_set",
                             nullptr);
  ...
  RNA_def_property_ui_text(
      ...
      "Number of recent frames averaged for viewport profiler timings");
  RNA_def_property_update(
      prop, NC_SCENE | ND_RENDER_OPTIONS, "rna_Scene_eevee_profiler_view_update");

  prop = RNA_def_property(srna, "use_performance_profiler_pause", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "performance_profiler_viewport_pause", 1);
  RNA_def_property_boolean_default(prop, false);
  RNA_def_property_ui_text(
      prop, "Pause", "Freeze displayed viewport profiler data without interrupting Eevee rendering");
  RNA_def_property_update(
      prop, NC_SCENE | ND_RENDER_OPTIONS, "rna_Scene_eevee_profiler_view_update");

  prop = RNA_def_property(srna, "use_performance_profiler_stage_list", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag", SCE_EEVEE_PERFORMANCE_PROFILER_STAGE_LIST);
  RNA_def_property_boolean_default(prop, false);
  RNA_def_property_ui_text(
      prop,
      "Show Stage List",
      "Show the detailed per-stage timing list in the viewport overlay and profiler panels");
  RNA_def_property_update(
      prop, NC_SCENE | ND_RENDER_OPTIONS, "rna_Scene_eevee_profiler_view_update");

  prop = RNA_def_property(srna, "use_performance_profiler_time_sort", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(
      prop, nullptr, "flag", SCE_EEVEE_PERFORMANCE_PROFILER_SORT_BY_TIME);
  RNA_def_property_boolean_default(prop, false);
  RNA_def_property_ui_text(
      prop, "Sort by Time", "Sort profiler stage lists by current frame CPU cost instead of pipeline order");
  RNA_def_property_update(
      prop, NC_SCENE | ND_RENDER_OPTIONS, "rna_Scene_eevee_profiler_view_update");

  prop = RNA_def_property(srna, "performance_profiler_viewport_summary", PROP_STRING, PROP_NONE);
  RNA_def_property_string_funcs(prop,
                                "rna_SceneEEVEE_performance_profiler_viewport_summary_get",
                                "rna_SceneEEVEE_performance_profiler_viewport_summary_length",
                                nullptr);
  RNA_def_property_clear_flag(prop, PROP_IDPROPERTY);
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(
      prop, "Viewport Performance Summary", "Latest Eevee viewport performance summary");

  prop = RNA_def_property(srna, "performance_profiler_viewport_report", PROP_STRING, PROP_NONE);
  RNA_def_property_string_funcs(prop,
                                "rna_SceneEEVEE_performance_profiler_viewport_report_get",
                                "rna_SceneEEVEE_performance_profiler_viewport_report_length",
                                nullptr);
  RNA_def_property_clear_flag(prop, PROP_IDPROPERTY);
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(
      prop, "Viewport Performance Report", "Latest Eevee viewport detailed stage report");

  prop = RNA_def_property(srna, "performance_profiler_render_report", PROP_STRING, PROP_NONE);
  RNA_def_property_string_funcs(prop,
                                "rna_SceneEEVEE_performance_profiler_render_report_get",
                                "rna_SceneEEVEE_performance_profiler_render_report_length",
                                nullptr);
  RNA_def_property_clear_flag(prop, PROP_IDPROPERTY);
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop, "Render Performance Report", "Latest Eevee final render performance report");
