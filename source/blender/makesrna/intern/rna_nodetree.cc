...
static void rna_NodeFilterMask_object_set(PointerRNA *ptr, PointerRNA value, ReportList * /*reports*/)
{
  ...
  ID *new_id = (id != nullptr && GS(id->name) == ID_OB &&
                OB_TYPE_IS_GEOMETRY(reinterpret_cast<Object *>(id)->type)) ?
                   id :
                   nullptr;
  if (node->id != nullptr) {
    id_us_min(node->id);
                                              ...
                                              ReportList * /*reports*/)
{
  bNode *node = ptr->data_as<bNode>();
  ID *id = static_cast<ID *>(value.data);
  ID *new_id = (id != nullptr && GS(id->name) == ID_GR) ? id : nullptr;
  if (node->id != nullptr) {
    id_us_min(node->id);
    node->id = nullptr;
  }
...
static bool rna_NodeFilterMask_renderable_object_poll(PointerRNA * /*ptr*/, PointerRNA value)
{
  Object *object = static_cast<Object *>(value.data);
  return object != nullptr && OB_TYPE_IS_GEOMETRY(object->type);
}

static PointerRNA rna_NodeFilterMaskItem_object_get(PointerRNA *ptr)
{
  NodeFilterMaskItem *item = static_cast<NodeFilterMaskItem *>(ptr->data);
  return (item->object != nullptr) ? RNA_id_pointer_create(&item->object->id) : PointerRNA_NULL;
}

static void rna_NodeFilterMaskItem_object_set(PointerRNA *ptr,
                                              PointerRNA value,
                                              ReportList * /*reports*/)
{
  NodeFilterMaskItem *item = static_cast<NodeFilterMaskItem *>(ptr->data);
  Object *object = static_cast<Object *>(value.data);
  Object *new_object = (object != nullptr && OB_TYPE_IS_GEOMETRY(object->type)) ? object : nullptr;
...
static void rna_NodeFilterMaskItem_update(Main *bmain, Scene * /*scene*/, PointerRNA *ptr)
{
...
static void rna_NodeTree_tag_changed(Main &bmain, bNodeTree &ntree)
{
                                    ...
                                    StringRefNull type)
{
  ...
  rna_NodeTree_tag_changed(*bmain, *ntree);
...
static const EnumPropertyItem *rna_NodeRaycastSampleAttributeItem_data_type_itemf(
    bContext * /*C*/, PointerRNA * /*ptr*/, PropertyRNA * /*prop*/, bool *r_free)
{
  *r_free = true;
  /* See #attribute_type_type_with_socket_fn. */
...
static void def_sh_filter_object_info(BlenderRNA * /*brna*/, StructRNA *srna)
{
  ...
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_ui_text(
      prop, "Object", "Object to read the world-space location from for filter materials");
  RNA_def_property_update(prop, NC_NODE | NA_EDITED, "rna_Node_update_relations");
}
...
static void def_sh_filter_object_mask(BlenderRNA * /*brna*/, StructRNA *srna)
{
  ...
  RNA_def_property_pointer_funcs(
      prop,
      "rna_NodeFilterMask_object_get",
      "rna_NodeFilterMask_object_set",
      nullptr,
      "rna_NodeFilterMask_renderable_object_poll");
  RNA_def_property_struct_type(prop, "Object");
  RNA_def_property_flag(prop, PROP_EDITABLE | PROP_ID_REFCOUNT);
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_ui_text(
...
static void rna_def_sh_raycast_sample_attribute_item(BlenderRNA *brna)
{
  StructRNA *srna = RNA_def_struct(brna, "NodeRaycastSampleAttributeItem", nullptr);
  RNA_def_struct_ui_text(srna, "Raycast Sample Attribute Item", "");
  RNA_def_struct_sdna(srna, "NodeRaycastSampleAttributeItem");

  rna_def_node_item_array_socket_item_common(srna, "RaycastSampleAttributeItemsAccessor", false);
  PropertyRNA *prop = RNA_def_property(srna, "data_type", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_items(prop, rna_enum_attribute_type_items);
  RNA_def_property_enum_funcs(
      prop, nullptr, nullptr, "rna_NodeRaycastSampleAttributeItem_data_type_itemf");
  RNA_def_property_ui_text(prop, "Data Type", "");
  RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
  RNA_def_property_update(prop,
                          NC_NODE | NA_EDITED,
                          "rna_Node_ItemArray_item_update<RaycastSampleAttributeItemsAccessor>");
}

static void rna_def_sh_raycast_sample_attribute_items(BlenderRNA *brna)
{
  StructRNA *srna = RNA_def_struct(brna, "NodeRaycastSampleAttributeItems", nullptr);
...
{
  rna_def_sh_raycast_sample_attribute_item(brna);
  rna_def_sh_raycast_sample_attribute_items(brna);
  ...
  RNA_def_struct_sdna_from(srna, "NodeShaderRaycast", "storage");

  prop = RNA_def_property(srna, "sample_attribute_items", PROP_COLLECTION, PROP_NONE);
  RNA_def_property_collection_sdna(
      prop, nullptr, "sample_attribute_items", "sample_attribute_items_num");
  RNA_def_property_struct_type(prop, "NodeRaycastSampleAttributeItem");
  RNA_def_property_ui_text(prop, "Items", "");
  RNA_def_property_srna(prop, "NodeRaycastSampleAttributeItems");
  ...
  RNA_def_property_pointer_funcs(
      prop,
      "rna_Node_ItemArray_active_get<RaycastSampleAttributeItemsAccessor>",
      "rna_Node_ItemArray_active_set<RaycastSampleAttributeItemsAccessor>",
      nullptr,
      nullptr);
  RNA_def_property_flag(prop, PROP_EDITABLE | PROP_NO_DEG_UPDATE);
  RNA_def_property_ui_text(prop, "Active Item Index", "Index of the active item");
...
static const EnumPropertyItem shader_portal_data_type_items[] = {
    {SOCK_FLOAT, "FLOAT", ICON_NODE_SOCKET_FLOAT, "Float", "Transmit a floating-point value"},
    {SOCK_VECTOR, "VECTOR", ICON_NODE_SOCKET_VECTOR, "Vector", "Transmit a vector value"},
    {SOCK_RGBA, "RGBA", ICON_NODE_SOCKET_RGBA, "Color", "Transmit a color value"},
    {SOCK_SHADER, "SHADER", ICON_NODE_SOCKET_SHADER, "Shader", "Transmit a shader closure"},
    {0, nullptr, 0, nullptr, nullptr},
};
...
static void def_sh_portal_in(BlenderRNA * /*brna*/, StructRNA *srna)
{
  ...
  RNA_def_struct_sdna_from(srna, "NodeShaderPortal", "storage");

  prop = RNA_def_property(srna, "portal_name", PROP_STRING, PROP_NONE);
  RNA_def_property_string_sdna(prop, nullptr, "name");
  RNA_def_property_ui_text(prop, "Name", "Portal identifier used to match portal outputs");
  RNA_def_property_update(prop, NC_NODE | NA_EDITED, "rna_Node_update");

  prop = RNA_def_property(srna, "data_type", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "data_type");
  RNA_def_property_enum_items(prop, shader_portal_data_type_items);
  RNA_def_property_ui_text(prop, "Type", "Data type carried by this portal");
  RNA_def_property_update(prop, NC_NODE | NA_EDITED, "rna_Node_socket_update");

  RNA_def_struct_sdna_from(srna, "bNode", nullptr);
}

static void def_sh_portal_out(BlenderRNA * /*brna*/, StructRNA *srna)
{
  PropertyRNA *prop;
  ...
  prop = RNA_def_property(srna, "portal_name", PROP_STRING, PROP_NONE);
  RNA_def_property_string_sdna(prop, nullptr, "name");
  RNA_def_property_ui_text(prop, "Name", "Portal identifier used to resolve the source input");
  RNA_def_property_update(prop, NC_NODE | NA_EDITED, "rna_Node_update");
...
static void def_sh_image_sample(BlenderRNA * /*brna*/, StructRNA *srna)
{
  static const EnumPropertyItem offset_type_items[] = {
      ...
      {SHD_IMG_SAMPLE_OFFSET_UV, "UV", 0, "UV", "Use the offset vector as absolute image UVs"},
      {0, nullptr, 0, nullptr, nullptr},
  };
...
static void def_sh_image_to_closure(BlenderRNA * /*brna*/, StructRNA *srna)
{
  static const EnumPropertyItem texture_type_items[] = {
      {IMA_IMAGE_TO_CLOSURE_TEXTURE_2D, "IMAGE_2D", 0, "2D Image", "Expose the image as a 2D sampler"},
...
static void def_sh_filter_mask_item(BlenderRNA *brna)
{
  ...
  RNA_def_property_pointer_funcs(
      prop,
      "rna_NodeFilterMaskItem_object_get",
      "rna_NodeFilterMaskItem_object_set",
      nullptr,
      "rna_NodeFilterMask_renderable_object_poll");
  RNA_def_property_flag(prop, PROP_EDITABLE | PROP_ID_REFCOUNT);
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_ui_text(prop, "Object", "Object included in the filter mask list");
  RNA_def_property_update(prop, NC_NODE | NA_EDITED, "rna_NodeFilterMaskItem_update");
...
static void rna_def_node_panel_state(BlenderRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;

  srna = RNA_def_struct(brna, "NodePanelState", nullptr);
  RNA_def_struct_sdna(srna, "bNodePanelState");
  RNA_def_struct_ui_text(srna, "Node Panel State", "Expansion state of a panel in a node");
  ...
  RNA_def_property_ui_text(prop, "Is Collapsed", "Whether the panel is collapsed");
  RNA_def_property_flag(prop, PROP_NO_DEG_UPDATE);
  RNA_def_property_update(prop, NC_NODE | ND_DISPLAY, nullptr);
}
...
static void rna_def_node(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna, "panel_states", PROP_COLLECTION, PROP_NONE);
  RNA_def_property_collection_sdna(prop, nullptr, "panel_states_array", "num_panel_states");
  RNA_def_property_struct_type(prop, "NodePanelState");
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop, "Panel States", "Expansion state of each panel in the node");
...
static void rna_def_nodes(BlenderRNA *brna)
{
  ...
  define("GeometryNode", "GeometryNodeInputInstanceReference");
