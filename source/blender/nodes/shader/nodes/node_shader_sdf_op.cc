...
static void node_shader_update_sdf_op(bNodeTree *ntree, bNode *node)
{
  ...
  /* Distance */
  bke::node_set_socket_availability(*ntree, *sockInputA, true);

  bke::node_set_socket_availability(*ntree, *sockInputB,
                            !ELEM(sdf->operation,
                                  SHD_SDF_OP_MASK,
                                  SHD_SDF_OP_INVERT,
                                  SHD_SDF_OP_DILATE,
  ...
  /* Values */
  bke::node_set_socket_availability(*ntree, *sockValue,
                            !ELEM(sdf->operation,
                                  SHD_SDF_OP_UNION,
  ...
  bke::node_set_socket_availability(*ntree, *sockCount, ELEM(sdf->operation, SHD_SDF_OP_ONION));
...
{
  NodeSdfOp *sdf = (NodeSdfOp *)MEM_new<NodeSdfOp>(__func__);
...
void register_node_type_sh_sdf_op()
{
  ...
  ntype.declare = file_ns::sh_node_sdf_op_declare;
  ...
  bke::node_type_storage(ntype, "NodeSdfOp", node_free_standard_storage, node_copy_standard_storage);
